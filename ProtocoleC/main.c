				...
				for (int i = 0; i < scene.lightsCount; i++) {
					...
					if (canTouchLight.empty == 1) {
						...
						if (strcmp(scene.light[i].type, "nor") == 0) {
							//if (strcmp(intersectedObject.type, "sph") == 0) //Si ce n'est pas le sol
					...
					else { // Si ca ne touche pas la lumière
						//if (intersectWithLight == 0) { // Si Aucune lumière n'avais été touché
							if (strcmp(intersectedObject.type, "gro") == 0) {
								col = AddColorInt(col, -60);
							}
						//}
					} 
				}

				// --------------------------------------------------------------------------- REFLEXION SUR LE SOL
				/*
					if (strcmp(intersectedObject.type, "gro") == 0) {
					Vector3 ref = SubVector(MultiplyVector(normale, 2 * ScalarVector(normale, ray.direction)), ray.direction);
					...
					for (int i = 0; i < scene.objectsCount; i++) { // Pour tout les objets de la scène
						...
						if (strcmp(scene.object[i].type, "sph") == 0) { // Si l'objet est une sphere
							tempIntersect = CollideWithSphere(ray2, scene.object[i].position, scene.object[i].radius);
							tempObj = scene.object[i];
						}
					...
					if (intersection.empty == 0) { // Si un objet à été touché
						col = AddColor(col, MultiplyColor(intersectedObject.color, 0.5));
					}
				}*/

				if(intersectWithLight == 1)
